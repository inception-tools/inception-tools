...
Houses the declaration of :py:class:`JsonSerializable` along with
...
from inceptiontools.serializable import Serializable, SerializationError
...
class JsonSerializable(Serializable, ABC):
    ...
    :py:class:`Serializable` by providing a way to marshal and unmarshal
    themselves from and to a JSON-like Python object representation.  The
    JSON-like Python object representation can be anything accepted by the
    :py:func:`json.dump` or produced by :py:func:`json.load`, i.e.,
    a composition of objects of the following types:
    ...
    def from_text_io(cls, fp: TextIOBase):
        ...
        package and delegating responsibility for unmarshalling the JSON
        representation to :py:meth:`from_json`.
        """
    ...
    def to_text_io(self, fp: TextIOBase):
        """
        Serializes this instance to a JSON-formatted text-based output
        stream by first converting it to a JSON-like Python object using the
        :py:mod:`json` package and delegating responsibility for serializing
        the JSON representation to the :py:mod:`json` package.
        """
